...
class SmartTVApp:
    def __init__(self, root):
        ...
        # ── Variables de Estado
        self.estado = "PRINCIPAL" # Puede ser "PRINCIPAL" o "VIDEOS"
        self.idx = 0
        self._filas = []
    ...
    def _build_ui(self):
        ...
        # Footer
        ftr = tk.Frame(self.root, bg=BG_HDR)
        ftr.grid(row=3, column=0, sticky="ew")
        tk.Frame(ftr, bg=BG_LINE, height=1).pack(fill=tk.X)
        row = tk.Frame(ftr, bg=BG_HDR)
        row.pack(fill=tk.X, padx=14, pady=5)
        for key, desc in [("UP/DN","Navegar"),("OK","Seleccionar"),("PREV","Regresar")]:
            tk.Label(row, text=f" {key} ", font=("Helvetica", 8, "bold"),
                     bg=GREEN, fg=WHITE, padx=2, pady=1).pack(side=tk.LEFT, padx=(0,3))
    ...
    def _build_sidebar(self, sb):
        ...
        for item in ITEMS:
            row = tk.Frame(sb, bg=BG_SB)
            row.pack(fill=tk.X)
            bar = tk.Frame(row, bg=BG_SB, width=5)
    ...
    def _refrescar_sidebar(self):
        for i, ui in enumerate(self._filas):
            sel   = (i == self.idx)
